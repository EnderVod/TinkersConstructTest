...
public class FancyItemFrameRenderer<T extends FancyItemFrameEntity> extends ItemFrameRenderer<T> {
  ...
  /** Keep explicit renderer references rather than relying on old access-transformer fields. */
  ...
  @Override
  protected int getBlockLightLevel(T frame, BlockPos pos) {
  ...
  public void render(T frame, float entityYaw, float partialTicks, PoseStack matrices, MultiBufferSource bufferIn, int packedLight) {
    ...
    // Reproduce vanilla's base entity-name handling because the normal item-frame renderer is being replaced.
    ...
    // Clear frames hide their frame geometry while displaying an item.
