...
public class CastingItemRenderTypeBuffer implements MultiBufferSource {
  ...
  public VertexConsumer getBuffer(RenderType type) {
    if (alpha < 255 && MAKE_TRANSPARENT.contains(type.name)) {
      // 1.21 hides CompositeRenderType texture state. Casting items are rendered from the item/block atlas,
      // so route opaque entity layers through the public translucent atlas render type instead.
