...
public class SolidFuelModule extends FuelModule {
  ...
  /** Current item handler at the fuel position. Refreshed through NeoForge's capability lookup. */
  @Nullable
  ...
  /**
   * Tries to consume fuel from the given item handler
  ...
  /** Fetches the current fuel handlers from the target position using native NeoForge capabilities. */
  private void fetchHandlers() {
    Level level = getLevel();
    // The old unsided BlockEntity#getCapability calls used a null direction, preserve that behavior here.
